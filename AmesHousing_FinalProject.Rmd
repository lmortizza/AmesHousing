...
```
If you change your roof type from the most common roof type to any other roof type, on average, your house will go down $`r abs(diff)`.
...
If you you go from an unfinished basement to any type of finished basement, our model predicts that on average, your house value will go down by $`r abs(diff)`. 
...
diff <- kitchen.prices[1] - price.comHouse

```

If you you make any upgrade to the kitchen from a kitchen with a quality 0- on average- your house value will go up by $`r abs(diff)`.
...
diff <- bath.prices[1] - price.comHouse

```

If you you make any number of full bathrooms above grade (when you started with zero) - on average- your house value will go up by $`r abs(diff)`.
