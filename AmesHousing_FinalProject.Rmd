...
```{r}
typeof(AmesHousing$Kitchen.Qual)
```
...
#Chosen Variables: lot_area, tot_rms_abv_grd, overall_qual, Kitchen.Qual, year_built, NeighborhoodNorthridge, Bsmt.QualEx
cv.NeighborhoodNorthridge <- smoothCV(x = training$NeighborhoodNorthridge, 
                    y = training$saleprice, 
                    df.min = 1, 
                    df.max = 10)
plot.smoothCV(cv.NeighborhoodNorthridge, 
              K = 10, 
              title.text = "CV Error: saleprice ~ NeighborhoodNorthridge", 
              y.scale.factor = 2)
```
