...
Examining RMSE and MAE for both the linear and gam $$models^{ii}$$, we can observe that for both metrics the gam model out performs the linear model. 
...
tot_rms_abv_grd <- Mode(training$tot_rms_abv_grd)

overall_qual <- Mode(training$overall_qual)

lot_area <- Mode(training$lot_area)

year_built <- Mode(training$year_built)

Kitchen.Qual <- Mode(training$Kitchen.Qual)

Neighborhood <- Mode(training$Neighborhood)

full_bath_abv_grd <- Mode(training$full_bath_abv_grd)

Roof.Style <- Mode(training$Roof.Style)

BsmtFin.Type.1 <- Mode(training$BsmtFin.Type.1)

test.df <- data.frame(tot_rms_abv_grd, overall_qual, lot_area, year_built, Bsmt.Qual, Kitchen.Qual, Neighborhood, full_bath_abv_grd, Roof.Style, BsmtFin.Type.1)
...
for  (i in roofstyle.list){
  ...
  Roof.Style <- c(i)
  BsmtFin.Type.1 <- Mode(training$BsmtFin.Type.1)
  list.cat <-c(tot_rms_abv_grd, overall_qual, lot_area, year_built, Bsmt.Qual, Kitchen.Qual, Neighborhood, full_bath_abv_grd, Roof.Style, BsmtFin.Type.1)
...
roof.df <- data.frame()
roof.df<-rbind(roof.df, roof.lst[1:10], roof.lst[11:20], roof.lst[21:30],roof.lst[31:40], roof.lst[41:50])
...
```{r}
price.comHouse - pred.roof
```
```{r}
basement.finish.list <- c("ALQ", "BLQ", "Rec", "LwQ", "Unf", "NA")
...
ggplotly(ggplot(AmesHousing, aes(saleprice, overall_qual)) +
  geom_point())
