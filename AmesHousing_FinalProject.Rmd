...
Neighborhood <- c('Somerset')

full_bath_abv_grd <- c(2)

Roof.Style <- c('Hip')

BsmtFin.Type.1 <- c('ALQ')
...
price.pred1 <- predict.Gam(gam.fit, test.df)

price.pred2 <- predict.Gam(gam.fit, test.df)
...
ggplot(AmesHousing, aes(saleprice, overall_qual)) +
  geom_boxplot()

ggplotly(ggplot(AmesHousing, aes(overall_qual, saleprice)) +
    scale_fill_viridis(discrete = TRUE, alpha=0.5) +
    geom_jitter(color=rgb(0.1,0.4,0.5,0.7), size=0.4, alpha=0.5) +
   geom_boxplot(fill = rgb(0.1,0.4,0.5,0.7))

summary(AmesHousing$overall_qual)
