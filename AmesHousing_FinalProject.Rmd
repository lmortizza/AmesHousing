...
AmesHousing <- AmesHousing %>%
  mutate(BsmtFin.Type.1 = ifelse(is.na(BsmtFin.Type.1), "No Basement", BsmtFin.Type.1))

AmesHousing$BsmtFin.Type.1 <- recode(AmesHousing$BsmtFin.Type.1, 
            `ALQ` =  "ALQ",
            `BLQ`  =  "BLQ",
            `GLQ`   =  "GLQ", 
            `LwQ`   = "LwQ",
...
as.factor(AmesHousing$BsmtFin.Type.1)
typeof(AmesHousing$BsmtFin.Type.1)
...
basement.df<-rbind(basement.df, basement.lst[1:10], basement.lst[11:20], basement.lst[21:30],basement.lst[31:40], basement.lst[41:50])

#Creates a list of the prices if we change the roof type in the "most common" house
basement.prices <- c()
for (i in 1:5){
  basement.prices[i] <- predict.Gam(gam.fit, as.vector(roof.df[i,]))
}
basement.prices
...
#Things to Change: basement.lst, basement.df, basement.prices
